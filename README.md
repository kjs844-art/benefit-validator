...
> 남은혜택은 사용자가 승인한 Gmail에서 혜택 후보 메일을 **읽기 전용**으로 확인합니다.
...
Gmail 분석은 "가입한 모든 사이트"를 보장하지 않습니다. 검색 대상 메일에서 근거를 찾은 서비스만
...
- Pretendard (UI) + IBM Plex Mono (숫자·날짜) — 디자인 토큰은 `src/styles.css`

## 디자인

화면을 수정하기 전에 [`DESIGN.md`](./DESIGN.md) 를 먼저 읽어 주세요. 색 토큰, 한국어 조판
규칙, 공용 컴포넌트 사용법과 하지 말아야 할 것이 정리되어 있습니다. Lovable에 그대로
전달할 수 있는 프롬프트도 문서 마지막에 있습니다.
