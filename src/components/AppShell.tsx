...
/**
 * Signed-in chrome: a fixed rail on desktop, a sticky bar with a scrolling tab
 * strip below 1024px. Active state is carried by activeProps/inactiveProps so
 * the two variants never put competing colour classes on the same element.
 */
const NAV = [
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[15.5rem_minmax(0,1fr)]">
      <aside className="sticky top-0 hidden h-screen flex-col border-r border-border bg-sidebar px-4 py-6 lg:flex">
        <Link to="/dashboard" className="px-2 text-[0.95rem]">
          <Wordmark />
        </Link>
        ...
        <div className="mt-auto border-t border-hairline pt-4">
          {email ? (
            <p className="truncate px-2.5 font-mono text-[0.6875rem] text-muted-foreground">
              {email}
            </p>
          ) : null}
          <button
            type="button"
            onClick={signOut}
            className="mt-1.5 flex w-full cursor-pointer items-center gap-2.5 rounded-md px-2.5 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"
          >
      ...
      <div className="flex min-h-screen flex-col">
        <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur lg:hidden">
          <div className="flex items-center justify-between gap-3 px-4 py-3">
            <Link to="/dashboard" className="text-[0.95rem]">
              <Wordmark />
            </Link>
            <button
              type="button"
              onClick={signOut}
              className="cursor-pointer rounded-md px-2 py-1.5 text-xs text-muted-foreground hover:bg-accent hover:text-foreground"
            >
              로그아웃
            </button>
          </div>
        ...
        <main className="mx-auto w-full max-w-5xl flex-1 px-5 py-8 lg:px-10 lg:py-12">
          {children}
        </main>
      </div>
    </div>
