...
export default defineTool({
  ...
  inputSchema: {
    service_id: z
      .string()
      .min(1)
      .describe("Target service id from list_benefits or create_service."),
    ...
    unit: z
      ...
      .min(1)
      .max(40)
      .describe("Unit exactly as the material states it, e.g. '개', 'GB', '회'."),
    granted_amount: z.number().nullish().describe("Amount granted per period; omit when unknown."),
    remaining_amount: z
      .number()
      .nullish()
      .describe("Remaining amount at observation time; omit when unknown."),
    monthly_cap: z.number().nullish().describe("Monthly cap stated by the material, if any."),
    extra_limit_note: z
      .string()
      .max(500)
      .nullish()
      .describe("Any additional restriction the material states."),
    reset_rule: z
    ...
    reset_anchor: z
      ...
      .max(80)
      .nullish()
      .describe("Reset anchor (e.g. '매월 1일') when stated."),
    observed_at: z
      .string()
      .min(1)
      .describe(
        "ISO 8601 timestamp of when the value was actually observed. Never fabricate a precise time: if the material only has a date, give midnight in observed_timezone and set observed_precision to 'day'.",
      ),
  ...
  handler: async (input, ctx) => {
    if (!ctx.isAuthenticated()) {
      throw new ToolError("Sign in as a KeyAtlas user to add data.");
    }
    ...
    if (!service) {
      throw new ToolError(
        "Service not found among your services. Use list_benefits or create_service first.",
      );
    }
    const { data, error } = await supabase
      ...
      .insert({
        ...
        reset_rule: input.reset_rule ?? "unknown",
        reset_anchor: input.reset_anchor ?? null,
        observed_at: input.observed_at,
        observed_precision: input.observed_precision ?? "minute",
        observed_timezone: input.observed_timezone ?? "Asia/Seoul",
        source_kind: "mcp",
